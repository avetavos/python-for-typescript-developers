...
`uv` (by Astral, the ruff team) is a Rust-powered replacement for pip and venv combined. It is [10–100× faster than pip](https://docs.astral.sh/uv/), handles virtual environments automatically, and installs interpreters too — so it is the default recommendation for new projects. Read layers 1 and 2 to understand what it is doing for you; reach for layer 3 in practice.
...
| Installs interpreters | no | no | yes (`uv python install`) |
| Maturity | very stable | stable (2.x) | stable, widely adopted |
