...
<Playground code={collectionsCode} />

<PyOnly title="Tuples: sequence ที่ immutable พร้อม unpacking">

tuple หน้าตาเหมือน array ที่ขนาดคงที่ แต่ความหมายต่างออกไป เพราะเป็น **immutable** จะ append ลบ หรือ reassign สมาชิกไม่ได้เลย ฝั่ง TypeScript มี `readonly` array กับ type แบบ `[T, U]` ก็จริง แต่ทั้งหมดนั้นเป็นแนวคิดระดับคอมไพเลอร์ ส่วน tuple ของ Python เป็น runtime type ที่มีตัวตนจริง
...
tuple ยังเป็นกลไกเบื้องหลัง **การ return หลายค่า** และ **การ unpacking** ด้วย:
...
export const quizCollections = [
  {
    q: 'collection ชนิดไหนของ Python ใกล้เคียง `Map<string, number>` ของ TypeScript มากที่สุด?',
  ...
  {
    q: 'method ไหนดึงค่าจาก dict ได้อย่างปลอดภัยโดยไม่โยน KeyError?',
    options: [
      'dict.fetch(key)',
  ...
  {
    q: 'เขียน `t = (1, 2, 3); t[0] = 99` แล้วเกิดอะไรขึ้น?',
    options: [
      'ค่าเปลี่ยนเป็น 99',
      'Python โยน TypeError ตอน runtime',
      'Python โยน SyntaxError',
      'Python แปลง tuple เป็น list ให้',
    ],
    answer: 1,
  },
];
